            ...
            >
                <div class="guild">
                    ...
                    <div class="details">
                        ...
                        <p>
                            Permissions (<a href="https://discordlookup.com/permissions-calculator/{guild.permissions}" target="_blank" rel="noopener noreferrer">View Calculator</a>):
                            {#each Object.keys(PERMISSIONS).filter(perm => (BigInt(guild?.permissions || '0') & BigInt(perm)) === BigInt(perm)) as perm, index (perm)}
                                <span class="chip">{formatName(PERMISSIONS[perm as unknown as keyof typeof PERMISSIONS])}</span>
                            {/each}
                        </p>
                        <p>
                            Features:
                            ...
                            {:else}
                                {#each guild.features as feature}
                                    <span class="chip">{formatName(feature)}</span>
                                {/each}
                            {/if}
                        </p>
                    </div>
